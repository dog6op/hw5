...
void coarsen(double *uf, double *uc, int N) {
    ...
    for (ic = 1; ic < N/2; ++ic)
        for (jc = 1; jc < N/2; ++jc)
            uc[ic*(N/2+1)+j] = 0.25 * uf[2*ic*(N+1)+2*jc] + 0.125 * (uf[(2*ic-1)*(N+1)+2*jc]+uf[(2*ic)*(N+1)+(2*jc-1)]+uf[(2*ic+1)*(N+1)+(2*jc)]+uf[(2*ic)*(N+1)+(2*jc+1)]) + 0.0625 * (uf[(2*ic-1)*(N+1)+(2*jc-1)]+uf[(2*ic-1)*(N+1)+(2*jc+1)]+uf[(2*ic+1)*(N+1)+(2*jc-1)]+uf[(2*ic+1)*(N+1)+(2*jc+1)]);
}
